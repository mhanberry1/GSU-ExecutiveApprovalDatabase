...
function autoload() {
	getUserInformation();
    var url_temp1 = "autoload-google-charts.jsp?time=" + new Date().getTime() + "&table=series_information";
...
function countryChanged() {
	...
	if (countryElement.selectedIndex > 0) {
		...
		ajaxGeneric("data-administrator-load.jsp?table=series_information&country=" + currentCountry, function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
...
function inputChange(input, row, column) {
	...
	var arguments = "table=series_information&country=" + currentCountry + "&series=" + rows[row][0]
	...
	if (numOfSelections > 0) {
		if (confirm("Are you sure you want to delete these " + numOfSelections + " rows?")) {
			...
			for (var i = toDelete.length - 1; i >= 0; i--) {
				...
				var arguments = "table=series_information&country=" + currentCountry + "&series=" + rows[index][0];
...
function insertData() {
	var arguments = "table=series_information&defaultCountry=" + currentCountry + "&data=" + document.getElementById("rowInsert").value;
	var returnFunction = function() {
		if (xmlhttp.readyState == 4){
			if (xmlhttp.status == 200) {
				checkAjaxResponse(xmlhttp.responseText);
